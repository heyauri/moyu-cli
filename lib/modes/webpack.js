...
var fakeBaseDir = __dirname.split(path.sep).map(function (i) { return words[Math.floor(Math.random() * words.length)]; }).join(path.sep);
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    st = new Date();
                    log("");
                    pm = words[Math.floor(Math.random() * words.length)];
